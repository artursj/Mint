...
class iPhorm_Element_File extends iPhorm_Element
{
    ...
    public function isValid()
    {
        ...
        if (!$this->getFileUploadValidator()->getRequired()) {
        ...
        $valid = apply_filters('iphorm_element_valid', $valid, null, $this);
        $valid = apply_filters('iphorm_element_valid_' . $this->getName(), $valid, null, $this);
    ...
    /**
     * Set whether to use the enhanced uploader
     *
     * @param boolean $flag
     */
    ...
    /**
     * Get whether to use the enhanced uploader
     *
     * @return boolean
     */
    ...
    public function getValueHtml($separator = '<br />')
    {
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $file) {
                if (isset($file['url'])) {
                    $str = '<a href="' . $file['url'] . '">' . $file['text'] . '</a>';
                } else {
                    $str = $file['text'];
                }
                $arr[] = $str;

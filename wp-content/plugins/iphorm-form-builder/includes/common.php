...
function iphorm_js_l10n()
{
    return array(
        ...
        'upload_too_many' => __('You have attempted to queue too many files', 'iphorm'),
        'upload_file_type_not_allowed' => __('This file type is not allowed', 'iphorm'),
        'upload_file_too_big' => __('This file exceeds the maximum upload size', 'iphorm'),
        'invalid_response' => __('The response from the server was invalid or malformed', 'iphorm'),
        'ajax_error' => __('Ajax error', 'iphorm'),
 ...
 */
function iphorm_process_upload_ajax()
{
    if (!iphorm_is_post_request()) {
        return;
    }

    if (!isset($_POST['iphorm_ajax_uploading']) || $_POST['iphorm_ajax_uploading'] != 1) {
        return;
    }

    // Validate the request data
    if (!isset($_POST['iphorm_id'], $_POST['iphorm_uid'], $_POST['iphorm_element_id']) ||
        !is_numeric($_POST['iphorm_id']) ||
        !is_string($_POST['iphorm_uid']) ||
        $_POST['iphorm_uid'] === '' ||
        !is_numeric($_POST['iphorm_element_id'])
    ) {
        iphorm_send_json(array(
            'type' => 'error',
            'message' => __('Bad request', 'iphorm')
        ));
    }

    $uid = preg_replace('/[^A-Za-z0-9]/', '', $_POST['iphorm_uid']);
    $form = iphorm_get_form($_POST['iphorm_id'], $uid);

    if (!($form instanceof iPhorm)) {
        iphorm_send_json(array(
            'type' => 'error',
            'message' => __('Could not find the form', 'iphorm')
        ));
    }
    ...
    if (!$form->getActive()) {
        iphorm_send_json(array(
            'type' => 'error',
            'message' => __('This form is not currently active', 'iphorm')
        ));
    }
    ...
    if (!($element instanceof iPhorm_Element_File)) {
        iphorm_send_json(array(
            'type' => 'error',
            'message' => __('Could not find the element', 'iphorm')
        ));
    }
    ...
    if (!isset($_FILES[$element->getName()])) {
        iphorm_send_json(array(
            'type' => 'error',
            'message' => __('File data not found', 'iphorm')
        ));
    }
    ...
    if (!is_writable($iphormTmpDir)) {
        iphorm_send_json(array(
            'type' => 'error',
            'message' => __('Temporary uploads directory is not writable', 'iphorm')
        ));
    }

    if ($element->isValid('')) {
        $sessionKey = 'iphorm-' . $uid;
...
add_action('init', 'iphorm_process_upload_ajax');
...
function iphorm_enqueue_scripts()
{
    ...
    if (!get_option('iphorm_disable_fileupload_output')) {
        wp_enqueue_script('blueimp-file-upload', iphorm_plugin_url() . '/js/jquery.fileupload.min.js', array('jquery-ui-widget'), '9.18.0', true);
    }
 ...
 * @return bool
 */
function iphorm_is_post_request()
{
 ...
 * @return mixed
 */
function iphorm_send_json($response)
{

...
jQuery.fn.select2Buttons = function(options) {
  return this.each(function(){
	...
	var buttonsHtml = $('<div class="react-select2Buttons"></div>');
	var selectIndex = 0;
	var addOptGroup = function(optGroup){
	  ...
	  optGroup.children('option').each(function(){
		var liHtml = $('<li title="' + $(this).html() + '"></li>').addClass($(this).val()); // TC addition - add option value as class on LI
		if ($(this).attr('disabled') || select.attr('disabled')){
		  liHtml.addClass('disabled');
		  liHtml.append('<span>' + $(this).html() + '</span>');
		}else{
		  liHtml.append('<a class="" href="#" data-select-index="' + selectIndex + '"><i class="fa"> </i></a>');
		}
